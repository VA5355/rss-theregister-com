...
@Component({
  selector: 'app-custom-pagination',standalone:true,
    imports: [  
    ReactiveFormsModule, CommonModule, FormsModule, NgClass ,
  NgIf], //RouterOutlet, BrowserModule,
  template: `
...
export class PaginationComponent implements OnChanges {
  ...
  @Input() page? = 1 ;//= undefined;   ;// = 1;
  @Input() pageSize? = 15;//  = undefined;   //= 15;
  @Input() total? = 0; //  = undefined;   //= 0;
  @Input() maxPages? = 10; //  = undefined;   //= 10;
  ...
  ngOnChanges(changes: SimpleChanges) {
    let re = changes['page']!.previousValue ;
    this.setPage(re ?this.page : 1) ;
  }

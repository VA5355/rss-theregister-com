...
export class AtomFeed {
  ...
  author: { uri: string| null  
    , name: string | null   , email: string | null }| null = null;
  entries: AtomEntry[] = [];
  ...
  private parseTopWords(html: string) {
    ...
    tmp.innerHTML = html;
    const words = tmp.textContent!.match(/\b(\w+)\b/g) ?  tmp.textContent!.match(/\b(\w+)\b/g) :  undefined; 
    const wordsMap:any = {};
    words!.forEach((word :string) => {
         let srtW = word +'';
         // if(this.common50.hasOwnProperty(srtW))
    //  let tt = this.common50.hasOwnProperty(srtW) !==undefined ? this.common50[srtW] : this.common50[word];
      if (this.common50.hasOwnProperty(srtW)) {
        return;
      }
      if (wordsMap.hasOwnProperty(srtW)) {
  ...
  private parseXML(xml:string) {
    ...
    let te = dom.getElementsByTagName('feed')!.item(0)!.textContent!
    this.parseTopWords(te);
    ...
    Array.from(entries).forEach(entry => {
      ...
      let te = this.elementValue(entry!, 'updated') ;
      let the = Date.parse(te   ? te : '')
      const e :any= {
